...
<div
    x-data="{
        ...
        imageFile: null,
        imageLoading: false,
        isActive: {{ old('is_active', $service->is_active ?? true) ? 'true' : 'false' }},
                ...
                @if (!empty($service?->image_path))
                    <div class="flex items-center gap-3 p-4 bg-gray-50 rounded-xl border-2 border-gray-200">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-sm font-medium text-gray-900 truncate">{{ basename($service->image_path) }}</p>
            ...
            <label class="flex items-center space-x-3 p-4 bg-white rounded-xl border-2 cursor-pointer transition-all hover:border-gray-300 hover:shadow-md active:scale-95 group" :class="isActive ? 'border-emerald-500 bg-emerald-50' : 'border-gray-200'">
                <div class="relative flex-shrink-0">
                    <input
                        ...
                        type="checkbox"
                        value="1"
                        @if(old('is_active', $service->is_active ?? true)) checked @endif
                        x-model="isActive"
                        class="sr-only" />
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-gray-500 mt-0.5">Show this service</p>
                </div>
            </label>
        </div>
    </div>
